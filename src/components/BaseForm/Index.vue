<template>
  <el-form
    ...
    :inline-message="(props && props['inline-message']) || false"
    ...
    <el-form-item
      ...
      <!-- 按钮/组 -->
      <template v-else-if="items[key].type === 'button'">
        <el-button
          v-for="(btn, i) in items[key].btns"
          :type="btn.type"
          :class="btn.class"
          :size="btn.size"
          ...
          :key="i"
          @click="() => {
            btn.onClick && btn.onClick(formName)
          }">
          {{btn.text}}
        </el-button>
      </template>
...
export default {
  ...
  mounted () {
    ...
    if (this.props && this.props.ref) {
      this.formName = this.$refs[this.props.ref]
    }
  },
  created () {},
  computed: {
